...
@test 'Use custom parameters' {
    ...
    diff $DIFF_ARGS <(UploadApplication) <(echo synthetics upload-application --config ./some/other/path.json --mobileApplicationId '123-123-123' --mobileApplicationVersionFilePath "example/test.apk" --versionName 'example 1.0' --latest)
}
...
@test 'Use default parameters' {
    ...
    diff $DIFF_ARGS <(UploadApplication) <(echo synthetics upload-application)
}
